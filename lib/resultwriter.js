    ...
    class ResultWriter {
        ...
        _formatItem(item) {
            _.each(item, (value, key) => {
                ...
                } else if (value instanceof Object && !this.objectFormatter) {
    ...
    class XmlWriter extends ResultWriter {
        ...
        _printList(list, listElement, rowElement, indent) {
            ...
            list.forEach(item => {
                ...
                this._printItem(item, colIndent, colFormat);
                console.log(rowEnd);
            });
            ...
            Object.keys(item)
                .forEach(key => {
                    ...
                    if (Array.isArray(value)) {
                        this._printList(value, key, 'item', colIndent);
                    }
                    else if (value instanceof Object) {
                        console.log(sprintf('%s<%s>', colIndent, key));
                        var itemColIndent = colIndent + '  ';
                        this._printItem(value, itemColIndent, itemColIndent + '<%s>%s</%1$s>');
                        console.log(sprintf('%s</%s>', colIndent, key));                        
                    }
                    else {
